...
class DB
{
	...
	if (!defined('TB_QUESTION')) {
            define('TB_QUESTION', self::$table_prefix.'question');
        }
        if (!defined('TB_QUIZ')) {
            define('TB_QUIZ', self::$table_prefix.'quiz');
        }
        if (!defined('TB_QUIZQUESTIONS')) {
            define('TB_QUIZQUESTIONS', self::$table_prefix.'quizquestions');
        }
        if (!defined('TB_ANSWER_HISTORY')) {
              define('TB_ANSWER_HISTORY', self::$table_prefix.'answerhistory');
        }	
    }
   ...
   /**
    ...
    public static function getQuestion(
        ...
        try {
            $query = 'SELECT *
                FROM `'.TB_QUESTION.'`
                WHERE `'.TB_QUESTION.'`.`id`=:question_id';

//          error_log("QUERY: " . $query ."\n", 3, "/tmp/error.log");

            $tokens[':question_id'] = $question_id;
...
/**
     ...
     */
    public static function getNumQuest($quiz) {
            // TODO: dependerá del quiz ID!
        if (!self::isDbConnected()) {
            return null;
        }

        try {
            $query = 'SELECT COUNT(*) as numQuest
                FROM `'.TB_QUIZQUESTIONS.'` where quizId=:quiz';
   ...
   /**
    ...
    public static function getAvailableQuizzes() {
        ...
        try {
                ...
                $query = "SELECT id, description, count(*) as numQuest
                        FROM ". TB_QUIZQUESTIONS." qq, ". TB_QUIZ ." q
   ...
   /**
     ...
     */
     ...
     public static function getCurrentQuestion(
        $question_pos, $user_id, $quiz_id
    ) {
        if (!self::isDbConnected()) {
            return null;
        }

        // error_log("QuestionID: " . $question_id . "UserID: " . $user_id . "\n", 3 , "/tmp/error.log");
        ...
        try {
            $query = 'SELECT * ,
                ...
                WHERE '.TB_USER_CHAT.'.`user_id`='.TB_USER.'.`id` AND
                      '.TB_USER_CHAT.'.`user_id`=:user_id AND
                      '.TB_USER_CHAT.'.`currentQuiz`=:quiz_id  AND
                      '.TB_USER_CHAT.'.`currentQuestion`=:questionPos  AND
                      '.TB_USER_CHAT.'.`currentQuestion`='.TB_QUIZQUESTIONS.'.`questionPos` AND
                      '.TB_QUESTION.'.`id`='.TB_QUIZQUESTIONS.'.`questionId` AND
                      '.TB_QUIZQUESTIONS.'.`quizId`=:quiz_id';

                // error_log("QUERY:". $query . "\n", 3, "/tmp/error.log");
                // error_log("quiz_id:". $quiz_id . "\n", 3, "/tmp/error.log");
                // error_log("question_pos:". $question_pos . "\n", 3, "/tmp/error.log");
                // error_log("user_id:". $user_id . "\n", 3, "/tmp/error.log");
            // TODO: user_id == chat_id ???
            $tokens[':quiz_id'] = isset($quiz_id)?$quiz_id:1;
            $tokens[':questionPos'] = $question_pos;
 ...
 /**
     ...
     * @return Gamer the gamer
     */
    public static function findGamer(
        $chat_id = null
    ) {
        if (!self::isDbConnected()) {
            return null;
        ...
        try {
            $query = 'SELECT * ,
                '.TB_USER_CHAT.'.`chat_id` AS `chat_id`,
                '.TB_USER.'.`username` AS `user_name`
                FROM `'.TB_USER_CHAT.'` LEFT JOIN `'.TB_USER.'`
                ON '.TB_USER_CHAT.'.`user_id`='.TB_USER.'.`id`
                WHERE '.TB_USER_CHAT.'.`user_id`=:chat_id';
 // error_log($query . "\n " . $chat_id . "\n", 3, "/tmp/error.log");
            $tokens[':chat_id'] = $chat_id;
...
/**
    ...
    public static function saveAnswer(
        ...
        try {
            $query = 'INSERT INTO
                    '.TB_ANSWER_HISTORY.' SET `question` = :question ,
                    `quiz` = :quiz ,
                    `user` = :user ,
                    `result` = :result,
                    `data` = NOW()';
          ...
          // error_log("QUERY:" . $query . "\n", 3, "/tmp/error.log");
          // error_log("QUERY:" . print_r($params,1) . "\n", 3, "/tmp/error.log");

            $params['result'] = $params['result']?"right":"wrong";

            $sth = self::$pdo->prepare($query);
            $sth->bindParam(':question', $params['question'], \PDO::PARAM_INT);
            $sth->bindParam(':quiz', $params['quiz'] , \PDO::PARAM_INT);
            $sth->bindParam(':user', $params['user'] , \PDO::PARAM_INT);
            $sth->bindParam(':result', $params['result'], \PDO::PARAM_STR);
  ...
  /**
     ...
     * @return null (no hay errores) o el texto del error
     */
    public static function saveGamer(
        $gamer
    ) {
        ...
        try {
            $query = 'UPDATE
                    '.TB_USER_CHAT.' SET `currentQuestion` = :currentQuestion ,
                    `rightAnswers` = :rightAnswers ,
                    `wrongAnswers` = :wrongAnswers , currentQuiz =:currentQuiz
                WHERE '.TB_USER_CHAT.'.`chat_id`=:chat_id AND ' .
                TB_USER_CHAT.'.`user_id`=:user_id'; // AND '. TB_USERS_CHATS .'.`currentQuestion`=:currentQuestion';

          // error_log("QUERY:" . $query . "\n", 3, "/tmp/error.log");
          // error_log("QUERY:" . $gamer->getCurrentQuestion() . "\n", 3, "/tmp/error.log");
          // error_log("QUERY:" . $gamer->getCurrentQuiz() . "\n", 3, "/tmp/error.log");

            $sth = self::$pdo->prepare($query);
            $sth->bindParam(':chat_id', $gamer->getChatId(), \PDO::PARAM_INT);
            $sth->bindParam(':user_id', $gamer->getChatId() , \PDO::PARAM_INT);
            $sth->bindParam(':currentQuiz', $gamer->getCurrentQuiz(), \PDO::PARAM_INT);
            $sth->bindParam(':currentQuestion', $gamer->getCurrentQuestion(), \PDO::PARAM_INT);
            $sth->bindParam(':rightAnswers', $gamer->getRightAnswers(), \PDO::PARAM_INT);
            $sth->bindParam(':wrongAnswers', $gamer->getWrongAnswers(), \PDO::PARAM_INT);
